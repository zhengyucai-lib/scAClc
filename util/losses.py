...
class NTXentLoss(nn.Module):
    def __init__(self, temperature=0.1, K=15):  # K为邻居数（不包含自身）
        super().__init__()
    ...
    def forward(self, features):
        ...
        n_anchor = n_samples // (self.K + 1)  # 每个锚细胞对应1自身+K邻居
        labels = torch.zeros(n_samples, dtype=torch.long, device=features.device)  # 标签数与样本数一致
