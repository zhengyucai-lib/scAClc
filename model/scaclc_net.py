import math
import torch.optim as optim  # 确认此导入存在
import numpy as np
import torch
...
class scAClc(nn.Module):
    def __init__(self, input_dim, device, z_dim=32,
                 encode_layers=[512, 256],
                 decode_layers=[256, 512],
                 activation='relu'):
        super(scAClc, self).__init__()
        self.z_dim = z_dim
        self.activation = activation
        self.encode_layers = encode_layers  # 新增：保存编码层参数
        self.decode_layers = decode_layers  # 新增：保存解码层参数
        self.input_dim = input_dim  # 新增：保存输入维度为实例属性
        # self.mu = None
        self.pretrain = False
        self.device = device
        self.alpha = 1.
...
# 在scAce模型中添加对比学习层
class scAClcContrastive(scAClc):
    def __init__(self, input_dim, device, z_dim=32, encode_layers=[512, 256], decode_layers=[256, 512], activation='relu'):
        # 调用父类初始化，传递所有参数
        super().__init__(
            input_dim=input_dim,
            device=device,
        ...
        self.contrastive_head = nn.Sequential(nn.Linear(self.z_dim, 128), nn.ReLU())
    ...
    def forward(self, x):
        ...
        q = self.soft_assign(z0)
        # 对比学习特征
        h = self.contrastive_head(z0)
        return z_mu, z_logvar, mu, disp, pi, q, h
